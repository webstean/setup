...
sudo sh -c 'SSH_ENV="$HOME/.ssh/agent-environment" >> /etc/profile.d/ssh-agent.sh'
sudo sh -c 'function start_agent { >> /etc/profile.d/ssh-agent.sh'
sudo sh -c 'echo "Initialising new SSH agent..." >> /etc/profile.d/ssh-agent.sh'
sudo sh -c '/usr/bin/ssh-agent | sed \'s/^echo/#echo/\' > "\${SSH_ENV}" >> /etc/profile.d/ssh-agent.sh'
sudo sh -c 'echo succeeded >> /etc/profile.d/ssh-agent.sh'
sudo sh -c 'chmod 600 "\${SSH_ENV}" >> /etc/profile.d/ssh-agent.sh'
sudo sh -c '. "\${SSH_ENV}" > /dev/null >> /etc/profile.d/ssh-agent.sh'
sudo sh -c '} >> /etc/profile.d/ssh-agent.sh'
sudo sh -c '# Source SSH settings, if applicable >> /etc/profile.d/ssh-agent.sh'
sudo sh -c 'if [ -f "\${SSH_ENV}" ]; then >> /etc/profile.d/ssh-agent.sh'
sudo sh -c '. "\${SSH_ENV}" > /dev/null >> /etc/profile.d/ssh-agent.sh'
sudo sh -c 'ps -ef | grep \${SSH_AGENT_PID} | grep ssh-agent$ > /dev/null || { >> /etc/profile.d/ssh-agent.sh'
sudo sh -c 'start_agent; >> /etc/profile.d/ssh-agent.sh'
sudo sh -c '} >> /etc/profile.d/ssh-agent.sh'
sudo sh -c 'else >> /etc/profile.d/ssh-agent.sh'
sudo sh -c 'start_agent; >> /etc/profile.d/ssh-agent.sh'
